...
const DEMO_REQUESTS = [
  {name:"Create Todo",method:"POST",url:"{{base_url}}/todos",headers:[{name:"Content-Type",value:"application/json"}],query:[],auth:{type:"none"},body:{title:"MoonProbe demo",completed:false,userId:1},timeout_ms:5000,assertions:[{type:"status_is",expected:201},{type:"json_exists",path:"$.id"}]},
  {name:"List Todos",method:"GET",url:"{{base_url}}/todos",headers:[],query:[{name:"_limit",value:"3"}],auth:{type:"none"},timeout_ms:5000,assertions:[{type:"status_is",expected:200},{type:"response_time_less_than",ms:3000}]},
  {name:"Get Todo",method:"GET",url:"{{base_url}}/todos/1",headers:[],query:[],auth:{type:"none"},timeout_ms:5000,assertions:[{type:"status_is",expected:200},{type:"json_equals",path:"$.id",expected:1}]},
  {name:"Update Todo",method:"PATCH",url:"{{base_url}}/todos/1",headers:[{name:"Content-Type",value:"application/json"}],query:[],auth:{type:"none"},body:{completed:true},timeout_ms:5000,assertions:[{type:"status_is",expected:200},{type:"json_equals",path:"$.completed",expected:true}]},
  {name:"Delete Todo",method:"DELETE",url:"{{base_url}}/todos/1",headers:[],query:[],auth:{type:"none"},timeout_ms:5000,assertions:[{type:"status_is",expected:200}]}
];
...
function currentRequest(){
  ...
  const request={name:DEMO_REQUESTS[state.selected]?.name||"Playground Request",method:el("method").value,url:el("url").value.trim(),headers:parseEditor("headers",[]),query:parseEditor("query",[]),auth:parseEditor("auth",{type:"none"}),timeout_ms:DEMO_REQUESTS[state.selected]?.timeout_ms||5000,assertions:parseEditor("tests",[])};
...
function resetResponse(){el("responseTitle").textContent="Ready";el("metrics").innerHTML="";el("responseBody").textContent="Press Send to execute this request.";el("assertions").innerHTML="";el("assertionSummary").textContent="—"}
function showResult(result){
  el("responseTitle").textContent=result.passed?"Request passed":"Request failed";
  ...
  el("assertionSummary").textContent=`${passed} / ${result.assertions.length} passed`;
  el("assertions").innerHTML=result.assertions.length?result.assertions.map(a=>`<div class="assertion ${a.passed?"pass":"fail"}"><span class="icon">${a.passed?"✓":"×"}</span><div><strong>${a.kind}</strong><small>${a.message} · expected ${escapeHtml(a.expected)} · actual ${escapeHtml(a.actual)}</small></div></div>`).join(""):'<div class="empty-state">No assertions configured.</div>';
}

async function sendCurrent(){
  const button=el("send");button.disabled=true;button.textContent="Sending…";
  try{const result=await executeRequest(currentRequest());showResult(result);renderRequestList()}
  catch(error){el("responseTitle").textContent="Adapter error";el("metrics").innerHTML='<span class="metric fail">ERROR</span>';el("responseBody").textContent=error.name==="AbortError"?"Request timed out.":String(error.message||error);el("assertions").innerHTML="";el("assertionSummary").textContent="not evaluated"}
  finally{button.disabled=false;button.textContent="Send"}
}

async function runCollection(){
  if(state.running)return;state.running=true;el("runCollection").disabled=true;el("runCollection").textContent="Running…";
  const rows=[];let passed=0;
  for(let i=0;i<DEMO_REQUESTS.length;i++){
    let row;
    try{const result=await executeRequest(DEMO_REQUESTS[i],i);if(result.passed)passed++;row={ok:result.passed,name:DEMO_REQUESTS[i].name,status:result.status,duration:result.duration_ms,note:result.assertions.find(x=>!x.passed)?.message||"all assertions passed"}}
  ...
  state.running=false;el("runCollection").disabled=false;el("runCollection").textContent="Run all";
}
...
function renderCollectionRows(rows,passed){
  ...
  const failed=rows.length-passed;el("collectionSummary").textContent=failed?`${passed} passed · ${failed} failed`:`${passed} / ${rows.length} passed`;
}
