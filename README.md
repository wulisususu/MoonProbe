...
> 当前状态：**Gate 1 Core Models 已完成**。Request / Response / Environment / Error 等基础模型已由 MoonBit 实现，并在 Linux / Windows CI 上通过 `moon check --deny-warn`、`moon test` 与 `moon build`。下一阶段进入 Template 与 Assertion Engine。
